<?php
$gTitle = LG_INVENTORY_CHART;
$gToolbox = '<a href="'.url('report/inventory').'"><i class="icon-table"></i> '.LG_INVENTORY_LIST.'</a>';
include tpl('header');
Load::js('amcharts');
?>
<form id="form1" class="form-inline">
    ...
    <select name="goodOrBad" class="selectpicker" id="goodOrBad">
        ...
        <option value="0">Good Warehouse</option>
        <option value="1">Defect Warehouse</option>
    </select>
...
$(document).ready(function(){
    $("#form1").submit(function(){
        $.getJSON("<?=url('report/inventoryChartData')?>", $(this).serializeArray(), function(result){
            // SERIAL CHART
