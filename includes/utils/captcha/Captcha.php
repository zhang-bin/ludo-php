...
/*@libk start */
     $basePath = LD_UTIL_PATH.'/captcha/';
    $this->audio_path =  $basePath.$this->audio_path;
    $this->gd_font_file =  $basePath.$this->gd_font_file;
    $this->ttf_file =  $this->ttf($basePath);
    $this->wordlist_file =  $basePath.$this->wordlist_file;
  }
  
  function ttf($basePath) {
  	$files = scandir($basePath.'/ttf');
  	$max = count($files) - 1;
	
  	while (true) {
  		$file = $files[rand(0, $max)];
  		if ($file != '.' && $file != '..' && strtolower(ext($file)) == 'ttf')  {
  			return $basePath.'/ttf/'.$file;
  		}
  	}
  }
/*@libk end */
